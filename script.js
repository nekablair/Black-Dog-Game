...
window.addEventListener('load', function() { 
    ...
    enemies.push(new Enemy(canvas.width, canvas.height))//runs only one time with page load, if we kept it in handleEnemies() if would push new enemies too many times
    ...
    function animate(timeStamp) {
        const deltaTime = timeStamp - lastTime //figuring out how many ms computer needs to give the next frame
        lastTime = timeStamp //setting this up so it can be used in the next loop and subtracted from new timeStamp when next loop starts
        ...
        player.update(input)
        handleEnemies(deltaTime) //calling function to update enemies
        //timestamp in deltaTime is generated by requestAnimationFrame(), the animate is always receiving a new time stamp due to it being called over and over again
        requestAnimationFrame(animate)//make endless animation loop by calling parent function
    }
    animate(0)//the first time it runs, it has no time stamp, so we must give it one, starting with zero
})